...
const FieldViewUsername = require('../fields/field-view-username');
const FieldViewPassword = require('../fields/field-view-password');
...
const BrowserExtensionView = require('./details-browser-extension-view');
...
const DetailsView = Backbone.View.extend({
    ...
    render: function () {
        ...
        if (!this.model) {
            const f = this.appModel.filter || {};
            const browserName = FeatureDetector.getBrowserExtensionType();
            this.$el.html(this.emptyTemplate(
                {
                    searchEmpty: f.text,
                    vaultEmpty: !f.text && f['*'] === null,
                    groupEmpty: !f.text && f.group && !f.subGroups,
                    groupTreeEmpty: !f.text && f.group && f.subGroups,
                    binEmpty: !f.text && f.trash,
                    colourEmpty: !f.text && f.color,
                    colourName: f.color === true ? Locale.coloured : f.color,
        ...
        this.addFieldViews();
        this.renderBrowserExtensionSettings();
        this.createScroll({
    ...
    addFieldViews: function() {
        ...
        this.passEditView = new FieldViewPassword({
            model: {
                name: '$Password',
            ...
            browserFieldModel: this.browserFieldFor('KeePass password'),
            displayGlobalPlaceholderOption: model.file.get('browserExtensionSettings').get('displayGlobalPlaceholderOption')
        });
        ...
        _.forEach(model.get('browserSettings').get('formFieldList').toArray(), function(value, field) {
            ...
            this.fieldViews.push(new FieldViewCustom({
                model: {
                    name: '$' + displayName,
                    title: displayName,
                    value: function() { return browserFieldModel.get('value'); }
                },
                browserFieldModel,
                displayGlobalPlaceholderOption: model.file.get('browserExtensionSettings').get('displayGlobalPlaceholderOption')
            }));
        ...
        this.fieldViews.forEach(function(fieldView) {
            ...
            if (hideEmptyFields) {
                ...
                if (!value || value.length === 0 || value.byteLength === 0) {
                    if (this.model.isJustCreated && ['$UserName', '$Password', '$URL'].indexOf(fieldView.model.name) >= 0) {
                        return; // don't hide basic fields for new records
                    }
                    fieldView.hide();
                }
            }
    ...
    browserFieldFor: function(displayName) {
        const settings = this.model.get('browserSettings');
        if (!settings) return;
        const collection = settings.get('formFieldList');
        if (!collection) return;
        const ffModel = collection.findWhere({displayName});
        return ffModel;
    },
    ...
    copyPassword: function() {
        return this.copyKeyPress(this.passEditView);
    },
    ...
    fieldChanged: function(e) {
        if (e.field) {
            if (e.field[0] === '$') {
                ...
                // If an browserFieldModel model is present, we will delete any
                // kdbx field unless it is the username or password
                const desiredValueSource = e.sender.getValueSource(fieldName);
                ...
                if (fieldName === 'otp') {
                    ...
                    fieldName = this.deduplicateFieldName(e.newField, desiredValueSource);
                    const allowEmpty = this.model.group.isEntryTemplatesGroup();
                    this.setField(fieldName, e.val, e.sender.browserFieldModel, desiredValueSource, allowEmpty);
                    this.entryUpdated();
                    return;
                    ...
                    this.setField(fieldName, e.val, e.sender.browserFieldModel, desiredValueSource);
                }
            ...
            // Remove the rendered views for all fields that have been deleted
            // and update where each view gets the field value from
            const collection = this.model.get('browserSettings').get('formFieldList');
            this.fieldViews.forEach(function(fieldView, ix) {
                if (fieldView instanceof FieldViewCustom && !fieldView.model.newField &&
                    ...
                    !collection.any({displayName: fieldView.model.title})) {
                    fieldView.remove();
                    this.fieldViews.splice(ix, 1);
                } else {
                    if (fieldView.cid === e.sender.cid && fieldView instanceof FieldViewCustom && !fieldView.model.newField) {
                        if (fieldView.getValueSource() === 'BrowserFieldOnly') {
                            fieldView.model.value = function() { return fieldView.browserFieldModel.get('value'); };
                        } else {
                            fieldView.model.value = () => { return this.model.fields[fieldView.model.title]; };
                        }
                    }
                    fieldView.update();
                }
    ...
    setField: function (name, value, browserFieldModel, desiredValueSource, allowEmpty) {
        switch (desiredValueSource) {
            case 'KdbxFieldOnly':
                const collection = this.model.get('browserSettings').get('formFieldList');
                const ffModel = collection.findWhere({displayName: name});
                if (ffModel) collection.remove(ffModel);
                this.model.setField(name, value, allowEmpty);
                break;
            case 'BrowserFieldOnly':
                let unprotectedValue = value;
                if (value.isProtected) {
                    unprotectedValue = value.getText();
                }
                this.model.setField(name, undefined);
                if (unprotectedValue) {
                    browserFieldModel.set('displayName', name);
                    browserFieldModel.set('value', unprotectedValue);
                    this.model.get('browserSettings').get('formFieldList').add(browserFieldModel);
                } else {
                    this.model.get('browserSettings').get('formFieldList').remove(browserFieldModel);
                }
                break;
            case 'BrowserAndKdbxField':
                browserFieldModel.set('displayName', browserFieldModel.get('type') === 'FFTusername' ? 'KeePass username' : 'KeePass password');
                browserFieldModel.set('value', browserFieldModel.get('type') === 'FFTusername' ? '{USERNAME}' : '{PASSWORD}');
                this.model.get('browserSettings').get('formFieldList').add(browserFieldModel);
                this.model.setField(name, value, allowEmpty);
                break;
        }
    ...
    removeOldField: function (name) {
        // These operations are idempotent so we just always do them both
        this.model.setField(name, undefined);
        const collection = this.model.get('browserSettings').get('formFieldList');
        const ffModel = collection.findWhere({displayName: name});
        if (ffModel) collection.remove(ffModel);
    ...
    deduplicateFieldName: function (requestedFieldName, desiredState) {
        const fieldExists = (name) => {
            if (desiredState === 'KdbxFieldOnly') {
                return this.model.hasField(name);
            } else {
                return this.model.get('browserSettings').get('formFieldList').where({name}).length > 0;
            }
        };

        let fieldName = requestedFieldName;
        let i = 0;
        while (fieldExists(fieldName)) {
            i++;
            fieldName = requestedFieldName + i;
        }
        return fieldName;
    },
    ...
    otpEnterManually: function() {
        if (this.model.fields.otp) {
            ...
            const fieldView = new FieldViewCustom({ model: {
                ...
                value: function() { return kdbxweb.ProtectedValue.fromString(''); }
            }});
            fieldView.on('change', this.fieldChanged.bind(this));
            fieldView.setElement(this.$el.find('.details__body-fields')).render();
            fieldView.edit();
    ...
    renderBrowserExtensionSettings: function() {
        if (this.views.extensionSettings) {
            this.views.extensionSettings.remove();
            delete this.views.extensionSettings;
            return;
        }
        this.views.extensionSettings = new BrowserExtensionView({
            model: this.model.get('browserSettings'),
            displayPriorityField: this.model.file.get('browserExtensionSettings').get('displayPriorityField')
        }).render();
        this.$el.find('.details__body-after').after(this.views.extensionSettings.el);
    }
});
